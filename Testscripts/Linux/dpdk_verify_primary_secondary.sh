...
function build_test_dpdk_primary_secondary () {
	...
	LogMsg "${RTE_SDK}/${EXAMPLES_DIR}/mp_server/${RTE_TARGET}/examples/mp_server -l0-1 -n4 $whitelist_params -- -p 0x14 -n2  2>&1 > $mp_server_log_file &"
	"${RTE_SDK}/${EXAMPLES_DIR}/mp_server/${RTE_TARGET}/examples/mp_server" -l0-1 -n4 $whitelist_params -- -p 0x14 -n2  2>&1 > $mp_server_log_file &
	sleep 30
	mp_client_log_file="./primary_secondary.log"
	LogMsg "timeout --preserve-status 30 ${RTE_SDK}/${EXAMPLES_DIR}/mp_client/${RTE_TARGET}/examples/mp_client -l3 \
		-n4 --proc-type=auto $whitelist_params -- -n 0 2>&1 > $mp_client_log_file"
	timeout --preserve-status 30 "${RTE_SDK}/${EXAMPLES_DIR}/mp_client/${RTE_TARGET}/examples/mp_client" -l3 \
		-n4 --proc-type=auto $whitelist_params -- -n 0 2>&1 > $mp_client_log_file
